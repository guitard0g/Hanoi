...
def iterativelySolve(fromPole, toPole):
	determiner=0
	if destinationPole==0:
		if poles[1][1]>poles[2][1]:
			determiner = (poles[1][1])%2
		else:
			determiner = (poles[2][1])%2
		
	if destinationPole==1:
		if poles[0][1]>poles[2][1]:
			determiner = (poles[0][1])%2
		else:
			determiner = (poles[2][1])%2
	if destinationPole==2:
		if poles[1][1]>poles[0][1]:
			determiner = (poles[1][1])%2
		else:
			determiner = (poles[0][1])%2
	global N
	...
	for i in range(1,N+1):
		if N%2==0:
			moveDefinitionArray.append((-1)**(i+determiner))
		if N%2==1:
			moveDefinitionArray.append((-1)**(i+determiner))
	poleOneTo = None
	poleTwoTo = None
